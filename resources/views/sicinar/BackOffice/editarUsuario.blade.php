                        ...
                        <div class="box-body">
                            ...
                            <div class="row">
                                <div class="col-xs-6 form-group">
                                    ...
                                    <select class="form-control m-bot15" name="cve_arbol" id="cve_arbol"  required>
                                    <option selected="true" disabled="disabled">Seleccionar Servidor Público </option>
                                    @foreach($regpersonal as $per)
                                        @if($per->folio == $user->cve_arbol)
                                            <option value="{{$per->folio}}" selected>{{$per->folio.' '.Trim($per->nombre_completo)}}</option>
                                        @else 
                                            <option value="{{$per->folio}}">{{$per->folio.' '.Trim($per->nombre_completo)}}</option>
                                        @endif                                    
                                    @endforeach
                                    </select>                           
                                </div>
                            ...
                            <div class="row">
                                <div class="col-xs-10 form-group">
                                    ...
                                    @foreach($reguadmon as $dep)
                                        @if($dep->uadmon_id == $user->depen_id)
                                            <option value="{{$dep->uadmon_id}}" selected>{{$dep->uadmon_id.' '.Trim($dep->uadmon_desc)}}</option>
                                        @else 
                                            <option value="{{$dep->uadmon_id}}">{{$dep->uadmon_id.' '.Trim($dep->uadmon_desc)}}</option>
                                        @endif                                    
                                    @endforeach
                                    </select>                           
                                </div>
