...
@section('title','Estadística por tipo de medios de comunicación')
                        ...
                        <table id="tabla1" class="table table-striped table-bordered table-sm">
                          <thead style="color: brown;" class="justify">
                            <tr>
                              ...
                              <th rowspan="2" style="text-align:left;"  >MEDIO DE COMUNICACIÓN</th>
                          ...
                          <tbody>
                            ...
                            @foreach($regnotamediod as $nota)
                              ...
                              <tr>
                                <td style="color:darkgreen;">{{$nota->medio_id}}</td>
                                <td style="color:darkgreen;">{{$nota->medio_desc}}   </td>
                                <td style="color:darkgreen; text-align:center;">{{$nota->total}}</td>
                                <td style="color:darkgreen; text-align:center;"><small>{{number_format($porcen,2)}}</small></td>
                              </tr>
                            @endforeach
                            @foreach($regnotamediot as $totales)
                               <tr>
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ...
          @foreach($regnotamediod as $tipogasto)
             ['{{$tipogasto->medio_desc}}', {{$tipogasto->total}} ],
          @endforeach
          //['Work',     11],
          //['Eat',      2],
          //['Commute',  2],
      ...
      function drawStuff() {
        var data = new google.visualization.arrayToDataTable([
          ...
          @foreach($regnotamediod as $tmes)
             ['{{$tmes->medio_desc}}', {{$tmes->total}} ],
          @endforeach          
        ]);

        var options = {                 
          title: 'Por medio informativo',
          width:  900,                // Alto de la pantalla horizontal
          height: 1200,               // Ancho de la pantalla '75%',
          colors: ['green'],          // Naranja #e7711c
          legend: { position: 'none' },
          chart: { title: 'Gráfica  ',
                   subtitle: 'Por medio informativo' },
          bars: 'horizontal',         // Required for Material Bar Charts.
          axes: {
            x: {
              0: { side: 'top', label: 'total'} // Top x-axis.
