...
@section('title','Estadística por nota calificación')
                      ...
                      <div class="box-body">
                        ...
                        <table id="tabla1" class="table table-striped table-bordered table-sm">
                          <thead style="color: brown;" class="justify">
                            <tr>
                              <th rowspan="2" style="text-align:left;"  >ID. </br>CALIF.    </th>
                              <th rowspan="2" style="text-align:left;"  >CALIFICACIÓN</br>NOTA RED SOCIAL</th>
                          ...
                          <tbody>
                            <?php $sumatotal = 0; ?>
                            @foreach($regnotaredest as $totales)
                            ...
                            @foreach($regnotaredesd as $nota)
                              <?php $i     = $i + 1;  ?>
                              <?php $porcen= ($nota->total*100)/$sumatotal; ?>                            
                              <tr>
                                @if ($nota->rs_calif == '1')
                                    <td style="color:darkgreen;">{{$nota->rs_calif}}   </td>
                                    <td style="color:darkgreen;">POSITIVA</td>
                                @else
                                    @if ($nota->rs_calif == '2')
                                        <td style="color:darkgreen;">{{$nota->rs_calif}}   </td>
                                        <td style="color:darkgreen;">NEUTRA</td>
                                    @else
                                        @if ($nota->rs_calif == '3')
                                            <td style="color:darkgreen;">{{$nota->rs_calif}}   </td>                                        
                                            <td style="color:darkgreen;">NEGATIVA</td>
                                        @endif                                                                        
                                    @endif                                    
                                @endif    
...
@section('javascrpt')
      ...
      function drawStuff() {
        ...
        var options = {
          ...
          chart: { title: 'Numeralia básica',
                   subtitle: 'Gráfica por calificación' },
  ...
  <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['Id. Calif.', 'Total'],
          @foreach($regnotaredesd as $tipogasto)
             ['{{$tipogasto->rs_calif}}', {{$tipogasto->total}} ],
          @endforeach
          //['Work',     11],
          //['Eat',      2],
          //['Commute',  2],
        ...
        var options = {
          title: 'Calificación de nota informativa',
  ...
  <script type="text/javascript">
      ...
      function drawStuff() {
        var data = new google.visualization.arrayToDataTable([
          ['Id. calif.', 'Total'],
          @foreach($regnotaredesd as $tmes)
             ['{{$tmes->rs_calif}}', {{$tmes->total}} ],
          @endforeach          
        ]);

        var options = {                 
          title: 'Por calificación de nota',
          width:  500,                // Alto de la pantalla horizontal
          height: 500,                // Ancho de la pantalla '75%',
          colors: ['green'],          // Naranja #e7711c
          legend: { position: 'none' },
          chart: { title: 'Gráfica  ',
                   subtitle: 'Por calificación de nota' },
          bars: 'horizontal',         // Required for Material Bar Charts.
          axes: {
            x: {
              0: { side: 'top', label: 'total'} // Top x-axis.
  ...
  <script type="text/javascript">
      ...
      function drawChart() {
        var data = google.visualization.arrayToDataTable([
          ['Id. calif.', 'Total'],
          @foreach($regnotaredesd as $placa)
             ['{{$placa->rs_calif}}', {{$placa->total}} ],
          @endforeach            
          //['Work',     11],
          //['Eat',      2],
          //['Commute',  2],
        ...
        var options = {
          title: 'Gráfica por calificación de nota',
          pieHole: 0.4,
          width: 500,                   // Ancho de la pantalla horizontal
          height: 300,                  // Alto de la pantall '75%',
        };
